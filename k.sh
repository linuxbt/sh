...
linux_ldnmp() {
  ...
  while true; do
    ...
    echo  "1. 安装LDNMP环境"
    echo  "${bai}------------------------${bai}"
    echo  "${lv}2. 安装WordPress${bai}"
    echo  "${lv}3. 安装Discuz论坛${bai}"
    echo  "${lv}4. 安装可道云桌面${bai}"
    echo  "${lv}5. 安装苹果CMS网站${bai}"
    echo  "${lv}6. 安装独角数发卡网${bai}"
    echo  "${lv}7. 安装flarum论坛网站${bai}"
    echo  "${lv}8. 安装typecho轻量博客网站${bai}"
    echo  "${lv}9. 安装雷池WAF旁入版${bai}"
    echo  "${lv}20. 自定义动态站点${bai}"
    echo  "------------------------"
    echo  "${huang}21. 仅安装nginx${bai}"
    echo  "${huang}22. 站点重定向${bai}"
    echo  "${huang}23. 站点反向代理-IP+端口${bai}"
    echo  "${huang}24. 站点反向代理-域名${bai}"
    echo  "${huang}25. 自定义静态站点${bai}"
    echo  "${huang}26. 安装Bitwarden密码管理平台${bai}"
    echo  "${huang}27. 安装Halo博客网站${bai}"
    echo  "------------------------"
    echo  "${lan}31. 站点数据管理${bai}"
    echo  "${lan}32. 备份全站数据${bai}"
    echo  "${lan}33. 定时远程备份${bai}"
    echo  "${lan}34. 还原全站数据${bai}"
    echo  "------------------------"
    echo  "${kjlan}35. 站点防御程序${bai}"
    echo  "------------------------"
    echo  "${hui}36. 优化LDNMP环境${bai}"
    echo  "${hui}37. 更新LDNMP环境${bai}"
    echo  "${hui}38. 卸载LDNMP环境${bai}"
    echo  "------------------------"
    echo  "0. 返回主菜单"
    echo  "------------------------"
    read -p "请输入你的选择: " sub_choice
