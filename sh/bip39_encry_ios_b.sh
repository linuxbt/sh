#!/bin/sh
# ==========================================
# iSH (Alpine Linux on iOS) Bootstrap
# ==========================================
...
# iSH 环境说明
############################
# iOS 设备依靠沙盒机制实现内存隔离，无法针对 Swap 做独立配置，
# 依赖于 iOS 的 Jetsam 机制，安全性有原生沙盒保障。
...
derive_key() {
    local pass="$1" salt="$2"
    # iSH 环境底层跨指令集模拟极耗性能，为防止 iOS 杀后台和等待过久，
    # 强制调整为适应 iOS 移动设备的轻量级安全参数 (-m 15 -> 32MB, -p 1 -> 单核)
    printf "%s" "$pass" | \
      argon2 "$salt" -id -t 3 -m 15 -p 1 -l 64 -r -q | \
      xxd -p | tr -d '\n'
}
...
main_menu() {
    while true; do
        ...
        echo -e "${CYAN}====================================================${NC}"
        echo -e "${YELLOW}                 【 iSH 移动端安全警告 】${NC}"
        echo -e "${GRAY} 🛡️  iOS App 沙盒特性已确保运行内存与其他 App 完全物理隔离。${NC}"
        echo -e "${RED} ⚠️  严禁执行系统截图，且操作时务必关闭设备的所有录屏功能！${NC}"
        echo -e "${CYAN}----------------------------------------------------${NC}"
