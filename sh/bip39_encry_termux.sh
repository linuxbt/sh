#!/usr/bin/env bash
# ==========================================
# Termux (Android) 环境初始化
# ==========================================
if ! command -v argon2 >/dev/null 2>&1 || ! command -v qrencode >/dev/null 2>&1 || ! command -v xxd >/dev/null 2>&1; then
    echo "▶ 首次运行：正在通过 pkg 下载安装必备组件 (argon2, openssl, xxd, qrencode)..."
    pkg update >/dev/null 2>&1
    pkg install ncurses-utils argon2 openssl xxd qrencode -y
    if [ $? -ne 0 ]; then
         echo "❌ 依赖安装失败！请检查 Termux 网络连接或更换源。"
         exit 1
    fi
fi
...
# Termux 环境说明
############################
# 安卓设备主要依靠 zRAM 和 LMK 机制调度，通常无需且无法无 Root 停用 Swap。
# Termux 应用本身受系统级沙盒保护，但剪贴板有被驻留 App 嗅探的可能风险。
...
derive_key() {
    local pass="$1" salt="$2"
    # Android 设备处理器采用原生 ARM64 架构运算，性能极其强悍。
    # 启用适用于高性能移动端的高标准参数：(-m 19 -> 512MB, -p 4 -> 并发4线程)
    printf "%s" "$pass" | \
...
menu_generate() {
    ...
    echo
    echo -e "${CYAN}----------------------------------------------------${NC}"
    echo -e "${YELLOW}【 密文二维码 】${GRAY}（可用常用扫码工具保存文本。Termux 支持双指缩放调整屏幕）${NC}"
    print_qr "$enc"
    echo -e "${CYAN}====================================================${NC}"
...
menu_decrypt() {
    ...
    echo -e "  ${GREEN}2.${NC} 在终端生成二维码 ${GRAY}(支持屏幕手指缩放。推荐用离线设备扫码提取)${NC}"
    echo -e "${CYAN}-----------------------------------${NC}"
    ...
    case "$o" in
        1) 
           ...
           echo -e "${RED}⚠️ 【 严重警告 】 请尽量手抄，由于这是安卓手机端，若必须复制明文，${NC}"
           echo -e "${RED}⚠️ 请确保您的后台没有国产毒留应用会随时窃取剪贴板记录！！${NC}\n"
           echo -e "${BOLD}${out}${NC}\n"
           ;;
        2) 
           echo -e "${YELLOW}请用不联网的备用设备扫码提取${NC}"
           echo -e "${GRAY}(非必要情况不要用微信等具备强烈分析意图的网络社交App扫码)：${NC}\n"
           print_qr "$out"
           ;;
        *)
...
main_menu() {
    while true; do
        ...
        echo -e "${CYAN}====================================================${NC}"
        echo -e "${YELLOW}               【 安卓 Termux 安全警告 】${NC}"
        echo -e "${GRAY} 🛡️  Termux 沙盒与内存隔离运行有效。无需修改系统 Swap。${NC}"
        echo -e "${RED} ⚠️  严禁执行系统截图，且操作时务必确保没有任何后台录屏 App 运行！${NC}"
        echo -e "${CYAN}----------------------------------------------------${NC}"
        echo -e "  ${GREEN}1.${NC} 🆕 生成并高强度原生框架加密助记词 ${GRAY}(推荐)${NC}"
        echo -e "  ${GREEN}2.${NC} 🔐 对已有保管的助记词加密保护"
        echo -e "  ${GREEN}3.${NC} 🔓 解密密文查阅或渲染二维码"
