...
for cred in "${CREDENTIALS[@]}"; do
    ...
    # 在远程服务器上执行指定的命令
    OUTPUTS["$user"]=$(sshpass -p "$pass" ssh -q -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ConnectTimeout=$SSH_TIMEOUT -t $user@$host "bash /home/你的用户名/sb_serv00.sh" 2>&1)
    if [ $? -ne 0 ];then
